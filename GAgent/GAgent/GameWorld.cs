...
namespace GAgent
{
    ...
    public delegate bool OutcomeIsValidDelegate(GameAction sourceEvent, Outcome sourceOutcome, List<GameEntity> entities);
    ...
    public class GameAction
    {
        ...
        public string ListOutcomes(List<Outcome> AllOutcomes, List<GameEntity> AllEntities)
        {
            ...
            if(ShowOutcomes)
            {
                Outcome[] validOutcomes = AllOutcomes.Where(o => o.IsValid(this, null, AllEntities)).ToArray();
                foreach (Outcome currOutcome in validOutcomes)
                {
                    sbOutput.AppendLine(currOutcome.Description);
                }
            }
            else
            {
        ...
        {
            Outcome[] validOutcomes = world.AllOutcomes.Where(o => o.IsValid(this, null, world.AllEntities)).ToArray();
